...
function slotMachine(cashoutAmount) {
    var coins = 25;
    ...
    while(coins > 0) {
        ...
        win = Math.floor(win);
        if (win == 50) {
            var payout = Math.random();
            payout = payout * (100-50) + 50;
            payout = Math.floor(payout);
            coins += payout;
            console.log("Jackpot!");
        }
        if (cashoutAmount < coins) {
            return("Winner winner chicken dinner! You now have " + coins + " coins. Keep the change you filthy animal.")
            break;
        } else {
            coins--;
        }
